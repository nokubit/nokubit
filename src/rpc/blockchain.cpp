...
// NOKUBIT: POS&FEE
extern void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry, bool include_hex = true);
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    // NOKUBIT: POS&FEE
    result.push_back(Pair("hashpos", blockindex->GetBlockPoSHash().GetHex()));
    int confirmations = -1;
...
void entryToJSON(UniValue &info, const CTxMemPoolEntry &e)
{
    ...
    info.push_back(Pair("sharingfee", ValueFromAmount(e.GetSharingFee())));
...
struct CCoinsStats
{
    ...
    uint64_t nDiskSize;
    // NOKUBIT: Nokubit AssetCoin
    asset_tag::CAmountMap nTotalAmount;

    // NOKUBIT: Nokubit AssetCoin
    CCoinsStats() : nHeight(0), nTransactions(0), nTransactionOutputs(0), nBogoSize(0), nDiskSize(0), nTotalAmount() {}
};
...
static void ApplyStats(CCoinsStats &stats, CHashWriter& ss, const uint256& hash, const std::map<uint32_t, Coin>& outputs)
{
    ...
    ss << outputs.begin()->second.fCoinType;
    ss << VARINT(outputs.begin()->second.nHeight);
    stats.nTransactions++;
    for (const auto output : outputs) {
        ss << VARINT(output.first + 1);
        ss << output.second.out.scriptPubKey;
        // NOKUBIT: Nokubit AssetCoin
        asset_tag::AssetAmount::AssetApplyStats(ss, output.second.out.nValue);
        stats.nTransactionOutputs++;
        stats.nTotalAmount += output.second.out.nValue;
        // NOKUBIT: Nokubit MemPool
        //stats.nBogoSize += 32 /* txid */ + 4 /* vout index */ + 4 /* height + coinbase */ + 8 /* amount */ +
        stats.nBogoSize += 32 /* txid */ + 4 /* vout index */ + 4 /* height */ + 1 /* coinype */ + 8 + 8 /* amount */ +
...
UniValue gettxout(const JSONRPCRequest& request)
{
    ...
    // NOKUBIT: Nokubit MemPool
    switch (coin.fCoinType) {
    case CoinTypePegIn:
        ret.push_back(Pair("pegin", true));
        break;
    case CoinTypeMintAsset:
        ret.push_back(Pair("mintasset", true));
        break;
    case CoinTypeCoin:
    case CoinTypeCoinBase:
