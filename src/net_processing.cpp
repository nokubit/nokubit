...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::GETHEADERS)
    {
        ...
        // NOKUBIT: POS&FEE
        int nFlags = 0;
        if (pfrom->nServices & NODE_POS)
            nFlags |= SERIALIZE_HEADER_POS;
        connman->PushMessage(pfrom, msgMaker.Make(nFlags, NetMsgType::HEADERS, vHeaders));
    }
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        // NOKUBIT: POS&FEE
        if (pfrom->nServices & NODE_POS)
            vRecv.SetVersion(SERIALIZE_HEADER_POS | vRecv.GetVersion());
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    // NOKUBIT: POS&FEE
                    int nFlags = 0;
                    if (pto->nServices & NODE_POS)
                        nFlags |= SERIALIZE_HEADER_POS;
                    connman->PushMessage(pto, msgMaker.Make(nFlags, NetMsgType::HEADERS, vHeaders));
                    state.pindexBestHeaderSent = pBestIndex;
                } else
                    fRevertToInv = true;
            }
