                         ...
                         CAmount& old_fee, CAmount& new_fee, CMutableTransaction& mtx)
{
    ...
    for (size_t i = 0; i < wtx.tx->vout.size(); ++i) {
        if (wallet->IsChange(wtx.tx->vout[i])) {
            ...
            // Questa deve essere una Fee
            if (wtx.tx->vout[i].nValue != asset_tag::COIN_ASSET)
                continue;
            if (nOutput != -1) {
    ...
    // NOKUBIT: Nokubit AssetCoin
    asset_tag::CAmountMap old_feeMap = wtx.GetDebit(ISMINE_SPENDABLE) - wtx.tx->GetValueOut();
    old_fee = old_feeMap[asset_tag::COIN_ASSET];
    ...
    if (poutput->nValue <= GetDustThreshold(*poutput, ::dustRelayFee)) {
        ...
        // NOKUBIT: Nokubit AssetCoin
        new_fee += poutput->nValue.GetAmountValue();
