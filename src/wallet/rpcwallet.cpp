...
{
    CAmount curBalance = pwallet->GetBalance()[asset_tag::COIN_ASSET];
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    std::map<CTxDestination, asset_tag::CAmountMap> balances = pwallet->GetAddressBalances();
...
UniValue movecmd(const JSONRPCRequest& request)
{
    ...
    // NOKUBIT: Nokubit AssetCoin
    asset_tag::AssetAmount nAmount = AssetAmountFromValue(request.params[2]);
    if (nAmount <= 0)
        throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount for send");
    ...
    if (!pwallet->AccountMove(strFrom, strTo, nAmount.GetAmountMap(), strComment)) {
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    // Check funds
    // NOKUBIT: Nokubit AssetCoin
    CAmount nBalance = pwallet->GetLegacyBalance(ISMINE_SPENDABLE, nMinDepth, &strAccount)[asset_tag::COIN_ASSET];
    if (nAmount > nBalance)
...
UniValue ListReceived(CWallet * const pwallet, const UniValue& params, bool fByAccounts)
{
    ...
    if (fByAccounts)
    {
        ...
        {
            // NOKUBIT: Nokubit AssetCoin
            asset_tag::CAmountMap nAmount = entry.second.nAmount;
...
UniValue listaccounts(const JSONRPCRequest& request)
{
    ...
    // NOKUBIT: Nokubit AssetCoin
    std::map<std::string, asset_tag::CAmountMap> mapAccountBalances;
    for (const std::pair<CTxDestination, CAddressBookData>& entry : pwallet->mapAddressBook) {
        if (IsMine(*pwallet, entry.first) & includeWatchonly) {  // This address belongs to me
            // NOKUBIT: Nokubit AssetCoin
            mapAccountBalances[entry.second.name] = asset_tag::CAmountMap();
        }
    }
    ...
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        // NOKUBIT: Nokubit AssetCoin
        if (nFee != 0)
            mapAccountBalances[strSentAccount] -= asset_tag::AssetAmount(asset_tag::COIN_ASSET, nFee);
        for (const COutputEntry& s : listSent)
    ...
    // NOKUBIT: Nokubit AssetCoin
    for (const std::pair<std::string, asset_tag::CAmountMap>& accountBalance : mapAccountBalances) {
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    // NOKUBIT: Nokubit AssetCoin
    asset_tag::CAmountMap nCredit = wtx.GetCredit(filter);
    asset_tag::CAmountMap nDebit = wtx.GetDebit(filter);
    asset_tag::CAmountMap nNet = nCredit - nDebit;
    CAmount nFee = (wtx.IsFromMe(filter) ? (wtx.tx->GetValueOut() - nDebit)[asset_tag::COIN_ASSET] : 0);

    // NOKUBIT: Nokubit AssetCoin
    entry.push_back(Pair("amount", ValueFromAmount(nNet - asset_tag::AssetAmount(asset_tag::COIN_ASSET, nFee))));
...
// NOKUBIT: Nokubit AssetCoin
extern UniValue issueasset(const JSONRPCRequest& request);
extern UniValue reissueasset(const JSONRPCRequest& request);
extern UniValue listissuances(const JSONRPCRequest& request);
...
{ //  category              name                        actor (function)           argNames
    ...
    // NOKUBIT: Nokubit AssetCoin
    { "wallet",             "issueasset",               &issueasset,               {"asset","destaddress","assetamount","fee","locktime","replaceable"} },
